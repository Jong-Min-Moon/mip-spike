...
from util_mip import tune_lambda_via_noise_constraint, jewell, evaluate_spike_inference_with_slack

def run_evaluation():
    # 1. Create results directory if it doesn't exist
    results_dir = 'evaluation_results_jewell'
    ...
    for name in dataset_names:
        result_path = os.path.join(results_dir, f"{name}_jewell.json")
        ...
        try:
            ...
            # Tune lambda using jewell
            # Note: jewell is imported from util_mip
            print(f"Tuning lambda for {name} using jewell...")
            optimal_lambda, best_s, best_z = tune_lambda_via_noise_constraint(y, jewell)
